...
#'
#' @description This function loads a mapping from genes to anatomical structures based on calls of expression in anatomical structures.
#' 
...
#'
#' @param species A character indicating species to be used.
#' Options are:
#'          "Anolis_carolinensis",
#'          "Bos_taurus",
...
#'          "Xenopus_tropicalis"
#'
#' @param datatype A vector of characters indicating data type(s) to be used.
#' Options are:
#'          "rna_seq",
#'          "affymetrix"
#'          "est",
...
#'
#' @param calltype A character of indicating the type of expression calls to be used for enrichment. Only calls for significant presence of expression are implemented. Over-expression calls, based on differential expression analysis, will be implemented in the future. 
#' Default is "present"
#'
#' @param stage A character indicating the targeted developmental stages for the analysis. Developmental stages can be chosen from the metastage ontology used in Bgee (available at \url{ftp://lausanne.isb-sib.ch/pub/databases/Bgee/current/stages.obo}). The ID, not the name of the metastage need to be used (prefix "BilaDO:"). 
#' Default is BilaDO:0000001, the root of the metastage ontology, meaning that expression data from all developmental stages will be used.
#'
#' @param confidence A character indicating if all expression calls should be retrieved, or only high quality expression calls
#' Options are:
#'          "all",
#'          "high_quality"
#' Default is "all"
