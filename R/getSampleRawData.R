#' @title Retrieve Bgee processed expression values for bulk RNA-seq, full length single-cell RNA-seq,
...
query_data = function(myBgeeObject, experimentId = NULL, sampleId = NULL, anatEntityId = NULL, 
  ...
  if(! (is.null(experimentId) & is.null(sampleId) & is.null(anatEntityId) & is.null(stageId)
    ...
    if (!is.null(sampleId)) {
      ...
      if(myBgeeObject$dataType %in% c("rna_seq", "sc_full_length", "sc_droplet_based")) {
    ...
    if(compareVersion(a = gsub("_", ".", myBgeeObject$release), b = "15.0") >= 0) {
      if (!is.null(cellTypeId)) {
        ...
        # Name of the column has been updated after Bgee 15.0
        cellTypeColumnName <- "Celltype.ID"
        if(compareVersion(a = gsub("_", ".", myBgeeObject$release), b = "15.0") == 0) {
          cellTypeColumnName <- "Cell.type.ID"
        }

        if(length(cellTypeId) == 1) {
          query <- paste0(query, "[", cellTypeColumnName, "] = \"", as.character(cellTypeId), "\"")
        } else {
          query <- paste0(query, "[", cellTypeColumnName, "] IN (\"",paste(as.character(cellTypeId), 
                                                               collapse="\", \""), "\")")
        }
      }
      if (!is.null(sex)) {
