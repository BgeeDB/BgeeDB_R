...
The ```getAnnotation()``` function will output the list of RNA-seq experiments and libraries available in Bgee for zebrafish. 
...
The ```getSampleRawData()``` function will download processed bulk RNA-seq data from all zebrafish experiments in Bgee as a data frame.

``` {r}
# download all RNA-seq experiments from zebrafish
data_bgee_zebrafish <- getSampleRawData(bgee)
...
data_bgee_zebrafish_gse68666 <- getSampleRawData(bgee, experimentId = "GSE68666")
```
...
# retrieve zebrafish RNA-Seq data for heart or brain
data_bgee_zebrafish_filters <- getSampleRawData(bgee, anatEntityId = c("UBERON:0000955","UBERON:0000948"))
# retrieve zebrafish RNA-Seq data for heart (UBERON:0000955) or brain (UBERON:0000948) part of the experiment GSE30617
data_bgee_zebrafish_filters <- getSampleRawData(bgee, experimentId = "GSE68666", anatEntityId = "UBERON:0000922")
# retrieve zebrafish RNA-Seq data for heart (UBERON:0000955) or brain (UBERON:0000948) from post-embryonic stage (UBERON:0000092)
data_bgee_zebrafish_filters <- getSampleRawData(bgee, stageId = "UBERON:0000113", anatEntityId = c("UBERON:0007132","UBERON:0006860"))
```
...
```{r}
# use only present calls and fill expression matric with TPM values
...
# download all RNA-seq experiments from zebrafish
cell_data_bgee_gallus_gallus <- getCellRawData(bgee, experimentId = "ERP132576", package = "anndata")
# isolate the first experiment
data_bgee_experiment1 <- data_bgee_zebrafish[[1]]
```

The result of the ```getCellRawData()``` depends on the package used to load the data. With \"zellkonverter\" it is an object of the class SingleCellExperiment. With \"anndata\" it is an object of the classe ... Each row is a gene and the expression levels are displayed as raw read counts, RPKMs (up to Bgee 13.2), TPMs (from Bgee 14.0), or FPKMs (from Bgee 14.0 to Bgee 14.2). A detection flag indicates if the gene is significantly expressed above background level of expression. From Bgee 15.0 a pValue allows to have a precise metric indicating how much the gene is significantly expressed above background level of expression (the detection flag is still available and a gene is considered as present if pValue < 0.05).
...
* more granular filtering using arguments in the getSampleRawData() function
